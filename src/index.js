...
class PhoneInput extends React.Component {
    ...
    constructor(props) {
        ...
        if (props.disableInitialCountryGuess) {
            ...
            // Default country
            countryGuess =
                onlyCountries.find(o => o.iso2 == props.country) || 0;
        } else {
            // Empty params
            countryGuess = 0;
        }
    ...
    guessSelectedCountry = memoize(
        (inputNumber, country, onlyCountries, hiddenAreaCodes) => {
            ...
            if (
                this.props.disableCountryGuessByNumber ||
                inputNumber.trim() === ""
            )
    ...
    handleFlagDropdownClick = e => {
        ...
        const allCountries = preferredCountries.concat(
        ...
        this.setState(
            prevState => ({
                ...
                highlightCountryIndex,
                searchValue: this.props.clearSearchOnOpen
                    ? ""
        ...
        if (this.numberInputRef) {
            if (
                ...
                this.setState(
                    ...
                    () => {
                        this.props.jumpCursorToEnd &&
                            setTimeout(this.cursorToEnd, 0);
                    }
                );
            }
        }
            ...
            if (/^\d+$/.test(sanitizedSearchValue)) {
                ...
                const searchedCountries = allCountries.filter(
                    ({ name, localName, iso2 }) =>
                        [`${name}`, `${localName || ""}`].some(field =>
                            field.toLowerCase().startsWith(sanitizedSearchValue)
                        )
                );

                const searchedCountriesByFuzzySearch = allCountries.filter(
                    ({ name, localName, iso2 }) =>
                        [`${name}`, `${localName || ""}`].some(field =>
                            field.toLowerCase().includes(sanitizedSearchValue)
                        )
                ...
                return [
                    ...new Set(
                        [].concat(
                            searchedCountries,
                            searchedCountriesByFuzzySearch
                        )
                    )
                ];
            }
    ...
    render() {
        ...
        return (
            ...
            >
                ...
                >
                    {renderStringAsFlag ? (
                        <div className={selectedFlagClasses}>
                            {renderStringAsFlag}
                        </div>
                    ) : (
                        <div
