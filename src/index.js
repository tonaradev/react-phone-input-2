...
class PhoneInput extends React.Component {
  ...
  guessSelectedCountry = memoize(
    (inputNumber, country, onlyCountries, hiddenAreaCodes) => {
      ...
      if (this.props.disableCountryGuessByNumber || inputNumber.trim() === "")
  ...
  updateCountry = (country) => {
    ...
    if (newSelectedCountry && newSelectedCountry.dialCode) {
      this.setState({
        ...
        formattedNumber: this.props.disableCountryCode
          ? ""
          : this.formatNumber(this.state.formattedNumber, newSelectedCountry),
      });
    }
  };
  ...
  handleFlagDropdownClick = (e) => {
    ...
    this.setState(
      (prevState) => ({
        ...
        highlightCountryIndex,
        searchValue: this.props.clearSearchOnOpen ? "" : prevState.searchValue,
      }),
      () => {
  ...
  getSearchFilteredCountries = () => {
    ...
    if (enableSearch && sanitizedSearchValue) {
      ...
      if (/^\d+$/.test(sanitizedSearchValue)) {
        ...
        return [
          ...new Set(
            [].concat(searchedCountries, searchedCountriesByFuzzySearch)
          ),
        ];
      }
    } else {
